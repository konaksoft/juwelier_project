...
def _build_multi_material_stock_aggregate():
    ...
    return {
        ...
        'diamond_value_tl': Coalesce(
            Sum(
                F('stock_pieces') * F('weighted_avg_cost_tl'),
                filter=_Q_DIAMOND,
                output_field=DecimalField(max_digits=20, decimal_places=2),
            ),
            ZERO2, output_field=DecimalField(max_digits=20, decimal_places=2),
...
def compute_daily_store_report(store, report_date):
    ...
    if report_date >= date.today() - timedelta(days=1):
        ...
        # ALTIN değerleri: mevcut stock_value_tl/hs alanlarını besler
        # (FAZ öncesi semantiği korunur — altın için aynı iş).
        stock_value_tl = stock_agg['gold_value_tl'] or ZERO2
        ...
        silver_stock_value_tl = stock_agg['silver_value_tl'] or ZERO2

        # Pırlanta
        diamond_stock_pieces = int(stock_agg['diamond_pieces'] or 0)
        diamond_stock_value_tl = stock_agg['diamond_value_tl'] or ZERO2

        # Saat
        watch_stock_pieces = int(stock_agg['watch_pieces'] or 0)
        watch_stock_value_tl = stock_agg['watch_value_tl'] or ZERO2
...
def get_dashboard_summary(store, target_date=None):
    ...
    Mevcut altın alanları ('stock_value_tl', 'stock_value_hs') dokunulmadı.
    """
    from apps.dashboard.models import DailyStoreReport
    ...
    result = {
        ...
        # Stok (Altın — mevcut)
        'stock_value_tl': report.stock_value_tl,
        ...
        'silver_stock_value_tl': report.silver_stock_value_tl,
        # FAZ C: Stok (Pırlanta)
        'diamond_stock_pieces': report.diamond_stock_pieces,
        'diamond_stock_value_tl': report.diamond_stock_value_tl,
        # FAZ C: Stok (Saat)
        'watch_stock_pieces': report.watch_stock_pieces,
        'watch_stock_value_tl': report.watch_stock_value_tl,
        # Meta
...
def get_date_range_summary(store, start_date, end_date):
    ...
    agg = (
        ...
        .aggregate(
            total_sales_tl=Coalesce(Sum('total_sales_tl'), ZERO2, output_field=DecimalField()),
            total_sales_hs=Coalesce(Sum('total_sales_hs'), ZERO4, output_field=DecimalField()),
            total_purchases_tl=Coalesce(Sum('total_purchases_tl'), ZERO2, output_field=DecimalField()),
            total_purchases_hs=Coalesce(Sum('total_purchases_hs'), ZERO4, output_field=DecimalField()),
            total_returns_tl=Coalesce(Sum('total_returns_tl'), ZERO2, output_field=DecimalField()),
            ...
            silver_stock_value_tl=Coalesce(Sum('silver_stock_value_tl'), ZERO2, output_field=DecimalField()),
            diamond_stock_pieces=Coalesce(Sum('diamond_stock_pieces'), Value(0)),
            diamond_stock_value_tl=Coalesce(Sum('diamond_stock_value_tl'), ZERO2, output_field=DecimalField()),
            watch_stock_pieces=Coalesce(Sum('watch_stock_pieces'), Value(0)),
            watch_stock_value_tl=Coalesce(Sum('watch_stock_value_tl'), ZERO2, output_field=DecimalField()),
        )
    )

    # Net hesaplamalar
    agg['net_sales_tl'] = agg['total_sales_tl'] - agg['total_purchases_tl'] - agg['total_returns_tl']
